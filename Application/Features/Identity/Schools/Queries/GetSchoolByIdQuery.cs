...
namespace Application.Features.Identity.Schools.Queries
{
	...
	public class GetSchoolByIdQueryHandler : IRequestHandler<GetSchoolByIdQuery , IResponseWrapper>
	{
        ...
        public async Task<IResponseWrapper> Handle(GetSchoolByIdQuery request, CancellationToken cancellationToken)
		{
            ...
            {
                var mappedSchool =  schoolInDb.Adapt<SchoolResponse>();
                return await ResponseWrapper<SchoolResponse>.SuccessAsync(mappedSchool);
            }
            return await ResponseWrapper<SchoolResponse>.FailAsync("School not found");
        }
	}
}
